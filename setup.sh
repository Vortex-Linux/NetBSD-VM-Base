...
while IFS= read -r command; do
    if [[ -n "$command" ]]; then
        tmux send-keys -t netbsd-vm-base "$command" C-m
        sleep 1
    fi
...
tmux send-keys -t netbsd-vm-base "$INSTALLATION_SCRIPT" C-m 
...
tmux send-keys -t netbsd-vm-base "$EXECUTE_INSTALL_SCRIPT" C-m
